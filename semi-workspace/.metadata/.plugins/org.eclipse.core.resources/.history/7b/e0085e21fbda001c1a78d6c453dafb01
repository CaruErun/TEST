...
<%
	ArrayList<FAQ> FAQList = (ArrayList<FAQ>)request.getAttribute("fList");
	PageInfo pi = (PageInfo)request.getAttribute("pi");
	String faqCate = (String)request.getAttribute("faqCate");
	...
	<div id="content_2">
		...
		<div id="content_2_6">
			<ul class="pagination">
				...
				<li class="page-item"><a class="page-link" href="<%=contextPath%>/SsearchFAQ.sc?cpage=<%=pi.getStartPage()-pi.getPageLimit()%>&select_faq_cate=<%=faqCate%>&search_user=<%=searchFaq%>">Previous</a></li>
				<%}%>
				<%for(int i=pi.getStartPage();i<=pi.getEndPage();i++) {%>
				<li class="page-item index-page"><a class="page-link" href="<%=contextPath%>/searchFAQ.sc?cpage=<%=i%>&select_faq_cate=<%=faqCate%>&search_user=<%=searchFaq%>"><%=i%></a></li>
				<%}%>
				<%if(pi.getEndPage()!=pi.getMaxPage()) {%>
				<li class="page-item"><a class="page-link" href="<%=contextPath%>/searchFAQ.sc?cpage=<%=pi.getEndPage()+1%>&select_faq_cate=<%=faqCate%>&search_user=<%=searchFaq%>">Next</a></li>
				<%} %>
			  </ul>
		</div>
		<!--글 목록 끝-->
