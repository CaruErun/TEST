...
pageEncoding="UTF-8" import="com.kh.common.PageInfo,com.kh.product.model.vo.*,java.util.ArrayList"%>
    ...
    int paging;
    String searchPd = (String)request.getAttribute("searchPd");
    %>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div id="content_2">
        <div id="body">
            ...
            <form action="searchProduct.pd" id="searchFrom">
                ...
                <input type="text" name="searchPd" id="searchPd" style="width: 300px;" >
	    ...
	    <div class="paging-area" align="center">
            <%if(currentPage!=1){ %>
                <button onclick="location.href='<%=contextPath%>/searchProduct.pd?cpage=<%=currentPage-1%>&searchPd=<%=searchPd%>'" class="btn btn-primary">&lt;</button>
                <%}else{ %>
                    <button onclick="location.href='<%=contextPath%>/searchProduct.pd?cpage=<%=currentPage-1%>&searchPd=<%=searchPd%>'" disabled class="btn btn-primary">&lt;</button>
                    <%} %>
                    
            <%for(paging=startPage; paging<=endPage; paging++){%>
            	<%if(paging!=currentPage){ %>
                    <button onclick="location.href='<%=contextPath%>/searchProduct.pd?cpage=<%=paging%>&searchPd=<%=searchPd %>'" class="btn btn-primary"><%=paging%></button>
                    <%}else{ %>
                        ...
                        <%if(currentPage!=maxPage){ %>
                            <button onclick="location.href='<%=contextPath%>/searchProduct.pd?cpage=<%=currentPage+1%>&searchPd=<%=searchPd%>'" class="btn btn-primary">&gt;</button>
                            <%}else{%>
                                <button onclick="location.href='<%=contextPath%>/searchProduct.pd?cpage=<%=currentPage+1%>&searchPd=<%=searchPd%>'" class="btn btn-primary" disabled>&gt;</button>
                                <%} %>
        </div>
    </div>                        
        <script>            
