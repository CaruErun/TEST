...
<%
	ArrayList<QNA> QNAList = (ArrayList<QNA>)request.getAttribute("qList");
	PageInfo pi = (PageInfo)request.getAttribute("pi");
	String qnaCate = (String)request.getAttribute("qnaCate");
	...
	<div id="content_2">
		...
		<div id="content_2_6">
			<ul class="pagination">
				...
				<li class="page-item"><a class="page-link" href="<%=contextPath%>/searchQNA.sc?cpage=<%=pi.getStartPage()-pi.getPageLimit()%>&select_qna_cate=<%=qnaCate%>&search_user=<%=searchQna%>">Previous</a></li>
				<%}%>
				<%for(int i=pi.getStartPage();i<=pi.getEndPage();i++) {%>
				<li class="page-item index-page"><a class="page-link" href="<%=contextPath%>/searchQNA.sc?cpage=<%=i%>&select_qna_cate=<%=qnaCate%>&search_user=<%=searchQna%>"><%=i%></a></li>
				<%}%>
				<%if(pi.getEndPage()!=pi.getMaxPage()) {%>
				<li class="page-item"><a class="page-link" href="<%=contextPath%>/searchQNA.sc?cpage=<%=pi.getEndPage()+1%>&select_qna_cate=<%=qnaCate%>&search_user=<%=searchQna%>">Next</a></li>
				<%} %>
			  </ul>
		</div>
		<!--글 목록 끝-->
