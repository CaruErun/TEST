...
<body>
    ...
    <div align="center" style="margin-bottom:50px;"><h2>FAQ</h2></div>
    <div id="content2">
...
<div class="paging-area" align="center">
            <%if(currentPage!=1){ %>
                <button onclick="location.href='<%=contextPath%>/FaqForm.fq?cpage=<%=currentPage-1%>'" class="btn btn-primary">&lt;</button>
                <%}else{ %>
                    <button onclick="location.href='<%=contextPath%>/FaqForm.fq?cpage=<%=currentPage-1%>'" disabled class="btn btn-primary">&lt;</button>
                    <%} %>
                    
            <%for(paging=startPage; paging<=endPage; paging++){%>
            	<%if(paging!=currentPage){ %>
                    <button onclick="location.href='<%=contextPath%>/FaqForm.fq?cpage=<%=paging%>'" class="btn btn-primary"><%=paging%></button>
                    <%}else{ %>
                        ...
                        <%if(currentPage!=maxPage){ %>
                            <button onclick="location.href='<%=contextPath%>/FaqForm.fq?cpage=<%=currentPage+1%>'" class="btn btn-primary">&gt;</button>
                            <%}else{%>
                                <button onclick="location.href='<%=contextPath%>/FaqForm.fq?cpage=<%=currentPage+1%>'"  class="btn btn-primary" disabled>&gt;</button>
                                <%} %>
        </div>
