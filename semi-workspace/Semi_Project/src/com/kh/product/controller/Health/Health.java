package com.kh.product.controller.Health;
...
import com.kh.product.model.service.Health.HealthService;
import com.kh.product.model.vo.Product;

/**
 * Servlet implementation class Soccer
 */
@WebServlet("/health.pg")
public class Health extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new HealthService().HealthCount(); 
		...
		int listCounttp = new HealthService().HealthtpCount(cateNo); 
		pageLimit = 10;
...
//		110 10 = 11 	  =  11 - 0 <- 						11.0   11.0
		...
		if((cateNo==0)) {
		ArrayList<Product> list = new HealthService().HealthProduct(pi);
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.setAttribute("category", "health");
		}
		else {
		ArrayList<Product> listtp = new HealthService().HealthtpProduct(pitp,cateNo);
