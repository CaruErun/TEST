...
/**
 * Servlet implementation class FaqFormController
 */
@WebServlet("/FaqForm.fq")
public class UserFAQFormController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UserFAQFormController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new ScService().selectUserFAQListCount();
		...
		ArrayList<FAQ> list= new ScService().selectUserFAQList(pi);
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
	request.getRequestDispatcher("/views/FAQ/userFAQ.jsp").forward(request, response);
